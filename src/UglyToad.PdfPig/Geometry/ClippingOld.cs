namespace UglyToad.PdfPig.Geometry
{
    ...
    /// 
    /// </summary>
    public static class ClippingOld
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public static IEnumerable<PdfPath> Clip(PdfPath clipping, PdfPath subject)
        {            
            if (clipping == null)
            {
            ...
            if (subject == null)
            {
                throw new ArgumentNullException(nameof(subject), "Clip(): the path to be clipped cannot be null.");
            }
            ...
            var pathRect = subject.GetBoundingRectangle();
            if (!pathRect.HasValue)
            {
                if (subject.Commands.Count == 1 && subject.Commands[0] is Move move)
                {
                    ...
                    {
                        throw new ArgumentException("Clip(): path bounding box not available.", nameof(subject));
                    }
                }
                else
                {
            ...
            if (pathRect.Value.Contains(clippingRect.Value) && subject.IsDrawnAsRectangle)
            {
                // TODO: check filling rule
            ...
            {
                if (subject.IsDrawnAsRectangle) // rectangle clipped
                {
                    if (subject.IsFilled || subject.IsClipping) // rectangle w/ rectangle, filled
                    {
                    ...
                    else // rectangle w/ rectangle, not filled
                    {
                        ...
                        foreach (var clipped in LiangBarsky(clippingRect.Value.Left, clippingRect.Value.Right, clippingRect.Value.Bottom, clippingRect.Value.Top, subject.Simplify(10)))
                        {
                            yield return clipped;
                        }
                    }
                ...
                {
                    if (subject.IsFilled || subject.IsClipping) // polygon with rectangle
                    {
                        if (subject.IsConvex())
                        {
                            Console.WriteLine("Clip: convex polygon clipped w/ rectangle -> Sutherland-Hodgman");
                            var clipped = SutherlandHodgman(clipping, subject);
                            if (clipped != null)
                            {
                                yield return clipped;
                            }
                        ...
                        else
                        {
                            ...
                            foreach (var clipped in GreinerHormann(clipping, subject))
                            {
                                yield return clipped;
                            }
                        }
            ...
            {
                if (subject.IsFilled || subject.IsClipping) // polygon with polygon
                {
                    if (clipping.IsConvex() && subject.IsConvex())
                    {
                ...
                else // polyline
                {
                    if (clipping.IsConvex())
                    {
                        ...
                        foreach (var clipped in CyrusBeck(clipping, subject.Simplify(10)))
                        {
                            yield return clipped;
                        }
                    }
            ...
            out double x0clip, out double y0clip, out double x1clip, out double y1clip)
        {
            ...
            for (int i = 1; i < clipping.Count; i++)
            {
                ...
                else
                {
                    ...
                    if (!GeometryExtensions.ccw(current, next, p1src) || 
                        !GeometryExtensions.ccw(current, next, p2src))
                    {
                        x0clip = double.NaN;
                        y0clip = double.NaN;
                        x1clip = double.NaN;
