...
{
    using Path = List<ClipperPoint64>;
    using Paths = List<List<ClipperPoint64>>;

    public struct ClipperPoint64
    {
        public Int64 X;
        public Int64 Y;
        public ClipperPoint64(Int64 X, Int64 Y)
        {
            this.X = X; this.Y = Y;
        }
        public ClipperPoint64(double x, double y)
        {
            this.X = (Int64)x; this.Y = (Int64)y;
        }

        public ClipperPoint64(ClipperPoint64 pt)
        {
            this.X = pt.X; this.Y = pt.Y;
        }

        public static bool operator ==(ClipperPoint64 a, ClipperPoint64 b)
        {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj is ClipperPoint64)
            {
                ClipperPoint64 a = (ClipperPoint64)obj;
                return (X == a.X) && (Y == a.Y);
            }
            else return false;
        }
    ...
    public enum ClipperClipType { None, Intersection, Union, Difference, Xor };
    public enum ClipperPathType { Subject, Clip };
    //By far the most widely used winding rules for polygon filling are
    //EvenOdd & NonZero (GDI, GDI+, XLib, OpenGL, Cairo, AGG, Quartz, SVG, Gr32)
    //Others rules include Positive, Negative and ABS_GTR_EQ_TWO (only in OpenGL)
    //see http://glprogramming.com/red/chapter11.html
    public enum ClipperFillRule { EvenOdd, NonZero, Positive, Negative };
    ...
    {
        internal ClipperPoint64 Pt;
        internal Vertex Next;
        internal Vertex Prev;
        internal VertexFlags Flags;
        public Vertex(ClipperPoint64 ip) { Pt.X = ip.X; Pt.Y = ip.Y; }
    }

    internal class LocalMinima
    {
        internal Vertex Vertex;
        internal ClipperPathType PathType;
        internal bool IsOpen;
    };

    public class Active
    {
        internal ClipperPoint64 Bot;
        internal ClipperPoint64 Curr;       //current (updated for every new Scanline)
        internal ClipperPoint64 Top;
        internal double Dx;
    ...
    public class PolyPath
    {
        ...
        private static void AddPolyNodeToPaths(PolyPath pp, Paths paths)
        {
            ...
            {
                Path p = new Path(cnt);
                foreach (ClipperPoint64 ip in pp.path) p.Add(ip);
                paths.Add(p);
            }
    ...
    public class ClipperPolyTree : PolyPath { };
    ...
    public class Clipper
    {
        ...
        private ClipperClipType ClipType;
        private ClipperFillRule FillType;
        ...
        private ClipperPoint64 GetIntersectPoint(Active edge1, Active edge2)
        {
            ClipperPoint64 ip = new ClipperPoint64();
            double b1, b2;
        ...
        private void AddLocMin(Vertex vert, ClipperPathType pt, bool isOpen)
        {
        ...
        private void AddPathToVertexList(Path p, ClipperPathType pt, bool isOpen)
        {
            int pathLen = p.Count;
        ...
        public void AddPath(Path path, ClipperPathType pt, bool isOpen = false)
        {
            if (isOpen)
            {
                if (pt == ClipperPathType.Clip)
        ...
        public void AddPaths(Paths paths, ClipperPathType pt, bool isOpen = false)
        {
            foreach (Path path in paths) AddPath(path, pt, isOpen);
        }
        //------------------------------------------------------------------------------

        private ClipperPathType GetPathType(Active e)
        {
        ...
        private bool IsContributingClosed(Active e)
        {
            ...
            switch (this.ClipType)
            {
                ...
                case ClipperClipType.Difference:
                    if (GetPathType(e) == ClipperPathType.Subject)
                        switch (this.FillType)
                        {
                            case ClipperFillRule.EvenOdd:
                            case ClipperFillRule.NonZero:
        ...
        private bool IsContributingOpen(Active e)
        {
            switch (this.ClipType)
            {
                case ClipperClipType.Intersection: return (e.WindCnt2 != 0);
                case ClipperClipType.Union: return (e.WindCnt == 0 && e.WindCnt2 == 0);
                case ClipperClipType.Difference: return (e.WindCnt2 == 0);
                case ClipperClipType.Xor: return (e.WindCnt != 0) != (e.WindCnt2 != 0);
            }
        ...
        {
            Active e2 = Actives;
            if (FillType == ClipperFillRule.EvenOdd)
            {
                int cnt1 = 0, cnt2 = 0;
                while (e2 != e)
                {
                    if (GetPathType(e2) == ClipperPathType.Clip) cnt2++;
            ...
            else
            {
                ...
                while (e2 != e)
                {
                    if (GetPathType(e2) == ClipperPathType.Clip) e.WindCnt2 += e2.WindDx;
        ...
        private void SetWindingLeftEdgeClosed(Active leftE)
        {
            ...
            ClipperPathType pt = GetPathType(leftE);
            ...
            else if (FillType == ClipperFillRule.EvenOdd)
            {
                leftE.WindCnt = leftE.WindDx;
        ...
        virtual protected void AddLocalMinPoly(Active e1, Active e2, ClipperPoint64 pt)
        {
        ...
        virtual protected void AddLocalMaxPoly(Active e1, Active e2, ClipperPoint64 Pt)
        {
            if (!IsHotEdge(e2))
        ...
        private void StartOpenPath(Active e, ClipperPoint64 pt)
        {
            OutRec outRec = CreateOutRec();
            outRec.IDx = OutRecList.Count;
            OutRecList.Add(outRec);
        ...
        virtual protected OutPt AddOutPt(Active e, ClipperPoint64 pt)
        {
        ...
        private void IntersectEdges(Active e1, Active e2, ClipperPoint64 pt)
        {
            ...
            {
                if (FillType != ClipperFillRule.EvenOdd) e1.WindCnt2 += e2.WindDx;
                else e1.WindCnt2 = (e1.WindCnt2 == 0) ? 1 : 0;
                if (FillType != ClipperFillRule.EvenOdd) e2.WindCnt2 -= e1.WindDx;
            ...
            if (IsHotEdge(e1) && IsHotEdge(e2))
            {
                if ((oldE1WindCnt != 0 && oldE1WindCnt != 1) || (oldE2WindCnt != 0 && oldE2WindCnt != 1) ||
                  (e1.LocalMin.PathType != e2.LocalMin.PathType && ClipType != ClipperClipType.Xor))
                {
                ...
                if (e1.LocalMin.PathType != e2.LocalMin.PathType)
                {
                    ...
                    switch (ClipType)
                    {
                        ...
                        case ClipperClipType.Union:
                            ...
                            break;
                        case ClipperClipType.Difference:
                            if (((GetPathType(e1) == ClipperPathType.Clip) && (e1Wc2 > 0) && (e2Wc2 > 0)) ||
                                ((GetPathType(e1) == ClipperPathType.Subject) && (e1Wc2 <= 0) && (e2Wc2 <= 0)))
                                AddLocalMinPoly(e1, e2, pt);
                            break;
                        case ClipperClipType.Xor:
                            AddLocalMinPoly(e1, e2, pt);
        ...
        virtual protected bool ExecuteInternal(ClipperClipType ct, ClipperFillRule ft)
        {
            if (ct == ClipperClipType.None) return true;
            FillType = ft;
            ClipType = ct;
            Reset();
            Int64 Y;
        ...
        virtual public bool Execute(ClipperClipType clipType, Paths Closed, ClipperFillRule ft = ClipperFillRule.EvenOdd)
        {
            try
            {
        ...
        virtual public bool Execute(ClipperClipType clipType, Paths Closed, Paths Open, ClipperFillRule ft = ClipperFillRule.EvenOdd)
        {
            try
            {
                if (Closed == null) return false;
        ...
        virtual public bool Execute(ClipperClipType clipType, ClipperPolyTree polytree, Paths Open, ClipperFillRule ft = ClipperFillRule.EvenOdd)
        {
            try
            {
        ...
        {
            ClipperPoint64 pt = GetIntersectPoint(e1, e2);
        ...
        *******************************************************************************/
        {
            ...
            while (true) //loops through consec. horizontal edges (if open)
            {
                ...
                while (e != null)
                {
                    ...
                    if (isLeftToRight)
                    {
                        pt = new ClipperPoint64(e.Curr.X, horz.Curr.Y);
        ...
        private void BuildResult2(ClipperPolyTree pt, Paths openPaths)
        {
            if (pt == null) return;
            if (openPaths != null)
            {
