namespace UglyToad.PdfPig.Tests.Dla
{
    ...
    public class KdTreeTests
    {
        ...
        public void BuildTree()
        {
            ...
            Assert.Equal(kdTree.Root.Element, new PdfPoint(7, 2), PointComparer);

            // left side
            Assert.Equal(kdTree.Root.LeftChild.Element, new PdfPoint(5, 4), PointComparer);

            Assert.Equal(kdTree.Root.LeftChild.LeftChild.Element, new PdfPoint(2, 3), PointComparer);
            Assert.Null(kdTree.Root.LeftChild.LeftChild.LeftChild);
            Assert.Null(kdTree.Root.LeftChild.LeftChild.RightChild);

            Assert.Equal(kdTree.Root.LeftChild.RightChild.Element, new PdfPoint(4, 7), PointComparer);
            Assert.Null(kdTree.Root.LeftChild.RightChild.LeftChild);
            Assert.Null(kdTree.Root.LeftChild.RightChild.RightChild);

            // right side
            Assert.Equal(kdTree.Root.RightChild.Element, new PdfPoint(9, 6), PointComparer);

            Assert.Null(kdTree.Root.RightChild.RightChild);

            Assert.Equal(kdTree.Root.RightChild.LeftChild.Element, new PdfPoint(8, 1), PointComparer);
