namespace UglyToad.PdfPig.Core
{
    ...
    public class PdfSubpath
    {
        ...
        /// <summary>
        /// True if the <see cref="PdfSubpath"/> was originaly draw as an axis aligned rectangle.
        /// </summary>
        ...
        /// <summary>
        /// Simplify this <see cref="PdfSubpath"/> by converting bezier curves in to <see cref="Line"/>s.
        /// </summary>
        /// <param name="n">Number of lines required (minimum is 1).</param>
        public PdfSubpath Simplify(int n = 4)
        {
            if (Commands.Count == 0)
            {
                return this;
        ...
        public void BezierCurveTo(double x2, double y2, double x3, double y3)
        {
            if (currentPosition.HasValue)
            {
                //shoeLaceSum += (x1 - currentPosition.Value.X) * (y1 + currentPosition.Value.Y);
                // need to check that
                shoeLaceSum += (x2 - currentPosition.Value.X) * (y2 + currentPosition.Value.Y);
                shoeLaceSum += (x3 - x2) * (y3 + y2);

                var to = new PdfPoint(x3, y3);
                commands.Add(new BezierCurve(currentPosition.Value, currentPosition.Value, new PdfPoint(x2, y2), to));
                currentPosition = to;
            }
            else
            {
